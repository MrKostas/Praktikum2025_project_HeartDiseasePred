...
1. Пользователь указывает путь к тестовому файлу в вэб форме
2. Сервис выполняет:
    * загрузку и обработку данных,
    * предсказания с помощью модели и сохранение результатов.
3. Результат предсказания сохраняется рядом с входным файлом (`predictions.csv` или `predictions.json`).